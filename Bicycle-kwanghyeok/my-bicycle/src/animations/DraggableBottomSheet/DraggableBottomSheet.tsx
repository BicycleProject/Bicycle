...
const DraggableBottomSheet = () => {
  ...
  const [otherUsersLocations, setOtherUsersLocations] = useState([
    { userId: 'user1', latitude: 37.5, longitude: 126.9},
    { userId: 'user2', latitude: 37.6, longitude:126.8},
  ]); // 웹소켓 다른 사용자 위치//////

  useEffect(() => {
    // 웹소켓 연결
    const socket = io('http://10.20.104.109:8088');

    // 실시간 위치 수신 이벤트 핸들러 등록
    socket.on('receiveLocation', (location) => {
      setOtherUsersLocations((prevLocations) => [...prevLocations, location]);
    });
  ...
  // 다른 사용자 마커
  const renderOtherUsersMarkers = () => {
    return otherUsersLocations.map((location, index) => (
      <Marker key={index} coordinate={{ latitude: location.latitude, longitude: location.longitude, }} title="다른 사용자" />
    ));
  };
